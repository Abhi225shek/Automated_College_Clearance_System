...
$pdf->Image('Tmslblack.jpg', 15, 12, 30); // x, y, width
...
$pdf->Cell(20, 8, strtoupper(htmlspecialchars($form['name'])), 'B', 1); // 'B' adds a bottom border
...
foreach ($college_id_chars as $char) {
    $pdf->Cell(8, 7, htmlspecialchars($char), 1, 0, 'C'); // Each character in a box
}
// Fill remaining boxes if ID is shorter than 10 characters (assuming 10 for consistency)
for ($i = count($college_id_chars); $i < 10; $i++) {
    $pdf->Cell(8, 7, '', 1, 0, 'C');
}
...
foreach ($session_chars as $char) {
    $pdf->Cell(8, 7, htmlspecialchars($char), 1, 0, 'C');
}
...
$pdf->Cell(110, 7, 'Student was studying in this Institute for Degree in (Stream):', 0);
$pdf->SetFont('helvetica', '', 10);
$pdf->Cell(20, 7, htmlspecialchars($form['stream']), 0, 1); // No 'B' for no underline

$pdf->Ln(10); // Line break before clearance sections

// --- Helper Function to Draw Each Section ---
function drawSection($pdf, $title, $description, $signature_label, $signature, $date) {
...
// Loop through each section and draw it if status data is available
foreach ($all_clearance_sections as $section_key) {
    ...
    // Use default/placeholder if status is not found or not approved
    $signature_val = $status_info['approved'] == 1 ? ($status_info['signature'] ?? '') : 'N/A';
    $updated_at_val = $status_info['approved'] == 1 ? ($status_info['updated_at'] ?? '') : '';

    drawSection(
        $pdf,
        ucfirst(str_replace('_', ' ', $section_key)) . ' Clearance Status:', // Title for the section
        $section_info['description'], // Description
        $section_info['signature_label'], // Signature label
        $signature_val, // Signature value
        $updated_at_val // Date value
    );
}
