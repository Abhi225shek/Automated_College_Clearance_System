...
if (isset($_POST['add_user'])) {
    ...
    if ($check_result->num_rows > 0) {
        echo "<div class='message-alert error'>Error: Username already exists!</div>";
    } else {
        try {
            ...
            if ($stmt->execute()) {
                echo "<div class='message-alert success'>User added successfully!</div>";
            } else {
                echo "<div class='message-alert error'>Error adding user: " . $stmt->error . "</div>";
            }
        } catch (mysqli_sql_exception $e) {
            // Catch database-related exceptions
            echo "<div class='message-alert error'>Database error: " . htmlspecialchars($e->getMessage()) . "</div>";
        }
    }
}
...
if (isset($_GET['delete'])) {
    ...
    if ($delete_id == $_SESSION['user_id']) {
        echo "<div class='message-alert error'>Error: You cannot delete your own account!</div>";
    } else {
        ...
        if ($stmt->execute()) {
            echo "<div class='message-alert success'>User deleted successfully!</div>";
        } else {
            echo "<div class='message-alert error'>Error deleting user: " . $stmt->error . "</div>";
        }
    }
}
...
if (isset($_GET['verify'])) {
    ...
    if ($stmt->execute()) {
        echo "<div class='message-alert success'>User verified successfully!</div>";
    } else {
        echo "<div class='message-alert error'>Error verifying user: " . $stmt->error . "</div>";
    }
}
?>
...
<head>
    ...
    <style>
        ...
        body {
            ...
            padding-top: 0;
            /* Remove extra padding at the top of the body itself */
            line-height: 1.6;
            display: flex;
        ...
        .admin-container {
            width: 100%;
            max-width: 1000px;
            /* Max width for the entire admin panel */
            margin: 0 auto;
        ...
        .admin-container h2 {
            ...
            /* Reduces space below the heading */
            position: relative;
        ...
        .section-card {
            ...
            /* Using accent for sections to differentiate */
            border-radius: var(--border-radius);
            padding: var(--spacing-md);
        ...
        .section-card h3::after {
            ...
            transform: translateX(-50%);
            width: 70px;
            /* Longer underline for section headings */
            height: 4px;
        ...
        .form-group {
            ...
            display: flex;
            /* Use flexbox for aligned labels and inputs */
            align-items: center;
            flex-wrap: wrap;
        ...
        .form-group select {
            ...
            min-width: 200px;
            /* Ensure a minimum width for inputs on larger screens */
        }
        ...
        .form-group select {
            max-width: calc(100% - 150px - var(--spacing-sm));
        ...
        table {
            ...
            width: 100%;
            min-width: 700px;
            /* Ensure table doesn't get too narrow, allowing overflow-x */
            box-shadow: var(--card-shadow);
            border-radius: var(--border-radius);
            overflow: hidden;
        ...
        @media (max-width: 768px) {
            ...
            .form-group select {
                width: 100%;
                max-width: 100%;
                /* Override max-width for selects */
            }
            ...
            table {
                min-width: unset;
                /* Remove fixed min-width for mobile stacking */
            }

            .table-responsive {
                overflow-x: auto;
                /* Keep horizontal scroll for larger tables if needed */
            }
            ...
            td:last-child .btn-action {
                margin: 5px;
                /* Adjust button margin within cell */
            }
        }
...
<body>
    ...
    <div class="admin-container">
        ...
        <!-- <script>
    // JavaScript function to show/hide the department dropdown based on role selection
    function toggleDepartmentField(role) {
        const departmentGroup = document.getElementById('department-group');
        const departmentSelect = document.getElementById('department');
        if (role === 'department') {
            departmentGroup.style.display = 'block'; // Use 'flex' to maintain form-group flex layout
        } else {
            departmentGroup.style.display = 'none';
            departmentSelect.value = ''; // Clear selection if department is not applicable
        }
    }
    // Call on page load to set initial state based on any pre-selected role (though currently not used for pre-selection)
    // You might uncomment this if you later implement pre-filling forms
    // document.addEventListener('DOMContentLoaded', function() {
    //     toggleDepartmentField(document.getElementById('role').value);
    // });
    </script> -->
            <script>
                function toggleDepartmentField(role) {
